...
@ViewEntity('teacher_lesson_grade_report_status', {
  expression: (dataSource: DataSource) =>
    dataSource
      ...
      .leftJoin(
        GradeWithReportMonth,
        'grades',
        'grades.teacherReferenceId = teachers.id AND grades.lessonReferenceId = lessons.id AND grades.reportMonthReferenceId = report_months.id',
      )
      .where(
        'COALESCE(lessons.start_date, report_months.endDate) <= report_months.endDate' +
