...
@ViewEntity("teacher_lesson_report_status", {
  expression: (dataSource: DataSource) => dataSource
    ...
    .leftJoin(AttReport, 'att_reports', 'att_reports.teacherReferenceId = teachers.id AND att_reports.lessonReferenceId = lessons.id AND att_reports.report_date <= report_months.endDate AND att_reports.report_date >= report_months.startDate')
