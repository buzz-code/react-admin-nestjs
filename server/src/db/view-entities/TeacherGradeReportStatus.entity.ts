...
import { TeacherLessonGradeReportStatus } from './TeacherLessonGradeReportStatus.entity';
import { getGroupConcatExpression } from '@shared/utils/entity/column-types.util';

@ViewEntity('teacher_grade_report_status', {
  expression: (dataSource: DataSource) =>
    dataSource
      .createQueryBuilder()
      .select(
        'CONCAT(COALESCE(tlrs.userId, "null"), "_", COALESCE(tlrs.teacherId, "null"), "_", ' +
          'COALESCE(tlrs.reportMonthId, "null"), "_", COALESCE(tlrs.year, "null"))',
        'id',
      )
      ...
      .addSelect(
        getGroupConcatExpression('CASE WHEN tlrs.isReported = 1 THEN tlrs.lessonId END', ',', true, 'tlrs.lessonName'),
        'reportedLessons',
      )
      .addSelect(
        getGroupConcatExpression('CASE WHEN tlrs.isReported = 0 THEN tlrs.lessonId END', ',', true, 'tlrs.lessonName'),
        'notReportedLessons',
      )
      .addSelect(
        getGroupConcatExpression(
          'CASE WHEN tlrs.isReported = 1 THEN tlrs.lessonName END',
          ', ',
          true,
          'tlrs.lessonName',
        ),
      ...
      .addSelect(
        getGroupConcatExpression(
          'CASE WHEN tlrs.isReported = 0 THEN tlrs.lessonName END',
          ', ',
          true,
          'tlrs.lessonName',
        ),
        'notReportedLessonNames',
      )
      .from(TeacherLessonGradeReportStatus, 'tlrs')
...
export class TeacherGradeReportStatus implements IHasUserId {
  @PrimaryColumn()
  @ViewColumn()
  id: string;
