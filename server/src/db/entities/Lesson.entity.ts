...
import {
  ...
  getDataSource,
} from '@shared/utils/entity/foreignKey.util';
...
export class Lesson implements IHasUserId {
  ...
  async fillFields() {
    ...
    try {
      ...
      const klassesArray =
        typeof this.klasses === 'string' && this.klasses.includes(',') ? this.klasses.split(',') : [this.klasses];
      this.klassReferenceIds = await findManyAndAssignReferenceIds(
        dataSource,
        Klass,
        { year: this.year, key: In(klassesArray) },
        this.userId,
        this.klassReferenceIds,
        this.klasses,
      );
