...
import {
  DataToExcelReportGenerator,
  getDateDataValidation,
  getIntegerDataValidation,
...
const getReportData: IGetReportDataFunction = async (
  ...
  const [user, teacher, teacherReportStatus, reportMonth] = await Promise.all([
    ...
    dataSource
      .getRepository(teacherReportTable)
      .findOneBy({ id: params.id, userId: params.userId, teacherReferenceId: Number(teacherId), year: Number(year) }),
  ...
  const lessonFilter: FindOptionsWhere<Lesson> = {
    id: In(teacherReportStatus.notReportedLessons),
  ...
  const [lessons] = await Promise.all([dataSource.getRepository(Lesson).findBy(lessonFilter)]);
  ...
  return lessons.map((lesson) => ({
    ...
    headerRow,
    formattedData: lessonStudents[lesson.id].map((sk) => [sk.klass.key, sk.student.id, sk.student.name]),
...
const getReportName = (data: TeacherReportFileData) =>
  `${data.fileTitle} למורה ${data.teacher?.name} לשיעור ${data.lesson?.name} - ${data.reportMonth?.name}`;
