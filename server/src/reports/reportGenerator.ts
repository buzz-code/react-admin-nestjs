...
export function getTeacherStatusFileReportParams(req: CrudRequest<any, any>): TeacherReportFileParams[] {
  ...
  const params = getAsArray(req.parsed.extra.ids)?.map((id) => ({
...
export async function sendTeacherReportFileMail(
  req: CrudRequest<any, any>,
  ...
  const getEmailParamsFromData = async (params: TeacherReportFileParams, data: TeacherReportFileData[]) => {
    ...
    const textParams = [
      data[0].teacher.name,
      data[0].teacherReportStatus.reportMonthName,
      data.map((item) => item.lesson.name).join(', '),
    ];
