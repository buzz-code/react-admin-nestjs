...
import { groupDataByKeys, groupDataByKeysAndCalc } from '@shared/utils/reportData.util';
...
const getReportData: IGetReportDataFunction = async (
  params: MichlolPopulatedFileParams,
  dataSource,
): Promise<MichlolPopulatedFileData> => {
  ...
  const studentTzs = params.michlolFileData
    .slice(3)
    .map((row) => row['B'])
    .filter(Boolean);
  ...
  if (lesson && students.length > 0) {
    ...
    const [reports, knownAbsences, attGradeEffect] = await Promise.all([
      ...
      dataSource
        .getRepository(AttGradeEffect)
        .find({ where: { userId: lesson.userId }, order: { percents: 'DESC', count: 'ASC' } }),
    ]);
  ...
  const specialFields = updatedData
    .map((row, index) => [
