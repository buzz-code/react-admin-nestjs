...
export function getReportDataFilterBySprAndDates(
  ids: string[],
  startDate: Date,
  endDate: Date,
): FindOptionsWhere<AttReportAndGrade>[] {
  return ids.map((id) => {
    const { studentReferenceId, teacherReferenceId, klassReferenceId, lessonReferenceId, userId, year } =
      breakSprId(id);
    return {
...
export function getKnownAbsenceFilterBySprAndDates(
  ids: string[],
  startDate: Date,
  endDate: Date,
): FindOptionsWhere<KnownAbsence> | FindOptionsWhere<KnownAbsence>[] {
...
export function getReportsFilterForReportCard(
  studentId: number,
  year: number,
  reportDateFilter: FindOperator<any>,
...
export function calcReportsData(
  data: AttReportAndGrade[],
  totalAbsencesData: { absnceCount: number }[],
