  ...
  protected async populatePivotData(pivotName: string, list: T[], extra: any, filter: any, auth: any) {
    ...
    switch (pivotName) {
      case 'PercentReportWithDates': {
        ...
        const totalAbsencesDataMap = groupDataByKeys(totalAbsencesData, [
          'studentReferenceId',
          'klassReferenceId',
          'lessonReferenceId',
          'userId',
        ...
        const estimatedLessonCountMap = groupDataByKeysAndCalc(estimatedLessonCount, ['id'], (items) => ({
          howManyLessons: calcSum(items, (item) => item.howManyLessons),
        }));

        Object.entries(sprMap).forEach(([key, val]) => {
          const reports = pivotDataMap[key] ?? [];
          const knownAbsKey = [
            val.studentReferenceId,
            val.klassReferenceId,
            val.lessonReferenceId,
            val.userId,
            val.year,
          ]
            .map(String)
          ...
          val.gradeAvg = getRelevantGrade(
            extra?.lastGrade,
            reportData.gradeAvg,
            reportData.lastGrade,
            reportData.maxGrade,
          );
        ...
        const gradeNames = await this.dataSource
          .getRepository(GradeName)
          .find({ where: { userId: getUserIdFromUser(auth) }, order: { key: 'DESC' } });
        const attendanceNames = await this.dataSource
          .getRepository(AttendanceName)
          .find({ where: { userId: getUserIdFromUser(auth) }, order: { key: 'DESC' } });
