...
export const Datagrid = ({ isAdmin, children, ...props }) => {
    const additionalBulkButtons = [
        ...
        isAdmin && (
            <BulkActionButton
                label="תיקון שיוך תלמידה v2"
                name="fixStudentReferenceV2"
                key="fixStudentReferenceV2"
                icon={<HelpIcon />}
                reloadOnEnd
            />
        ),
    ...
    return (
        <CommonDatagrid {...props} additionalBulkButtons={additionalBulkButtons}>
            ...
            <MultiReferenceField
                source="klassReferenceId"
                label="resources.att_report_with_report_month.fields.klass.klassTypeReferenceId"
                sortable={false}
                optionalSource="klassId"
                reference="klass"
                optionalTarget="key"
            ...
            {hasReportGroupPermission && (
                <ReferenceField
                    label="נושא"
                    source="reportGroupSessionId"
                    reference="report_group_session"
                    link={false}
                >
