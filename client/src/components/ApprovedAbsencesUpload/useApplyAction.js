...
export const useApplyAction = (previewData, configValues, setLoading, resetForm) => {
    ...
    const handleApply = useCallback(async () => {
        ...
        try {
            ...
            const matchedReports = previewData.details.filter((d) => d.matched).flatMap((d) => d.reports);
            ...
            const response = await dataProvider.action('att_report_with_report_month', 'bulkKnownAbsences', {
            ...
            if (previewData.notMatched > 0) {
                setTimeout(() => {
                    notify(`${previewData.notMatched} שורות דולגו בגלל שגיאות`, { type: 'warning' });
                }, 1000);
            }
