    ...
    return (
        <ListContextProvider value={listContext}>
            <ResourceContextProvider value="approved_absences_preview">
                <Datagrid bulkActionButtons={false} rowClick={false} expand={<ExpandedRow />}>
                    <FunctionField
                        label="סטטוס"
                        render={(record) => {
                    ...
                    />
                    <FunctionField label="מספר כיתה" render={(record) => record.klassKey} />
                    <FunctionField label="שם כיתה" render={(record) => record.klassName || 'לא נמצא'} />
                    <FunctionField label="תאריך" render={(record) => record.row.reportDate} />
                    <FunctionField label="דיווחים" render={(record) => `${record.reports?.length || 0} דיווחים`} />
                    <FunctionField label="הערות" render={(record) => record.error || record.warning || '-'} />
                </Datagrid>
            </ResourceContextProvider>
        </ListContextProvider>
    );
